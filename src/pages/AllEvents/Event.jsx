...
const VitalCard = ({ label, display, numericValue, thresholdKey, forceVariant, icon: Icon, children, sx = {} }) => {
  ...
  return (
    <Box sx={{
      height: CARD_H, borderRadius: "8px", padding: "10px 13px",
      boxSizing: "border-box", display: "flex", flexDirection: "column",
      justifyContent: "space-between", flex: 1, minWidth: 0,
      background: bgOf(v), ...borderOf(v), ...sx,
    }}>
...
const PatientVitalsSidebar = ({ open = false, onClose, patientData = DEFAULT_VITALS }) => {
  ...
  return (
    <>
      {/* Backdrop — zIndex 10000 so it sits above everything in AllEvents */}
      {open && (
        <Box
          onClick={onClose}
          sx={{
      ...
      {/* Sidebar panel — zIndex 10001 so it sits above the backdrop */}
      <Slide direction="left" in={open} mountOnEnter unmountOnExit timeout={300}>
        <Box sx={{
          ...
          <Box sx={{
            ...
            <Row>
              <VitalCard label="Heart Rate" display={vitals.heartRate.display} numericValue={vitals.heartRate.value} thresholdKey="heartRate" icon={HeartRateIcon} />
              <VitalCard label="Sweating" display={vitals.sweating.display} icon={SweatingIcon} />
            </Row>
            <Row>
              <VitalCard label="Blood Pressure" display={vitals.bloodPressure.display} icon={BloodPressureIcon} />
              <VitalCard label="ECG" display={vitals.ecg.display}><ECGIcon /></VitalCard>
            </Row>
            <Row>
              <VitalCard label="Oxygen" display={vitals.oxygen.display} numericValue={vitals.oxygen.value} thresholdKey="oxygen" icon={OxygenIcon} />
              <VitalCard label="Pain Score" display={vitals.painScore.display} numericValue={vitals.painScore.value} thresholdKey="painScore" icon={PainScoreIcon} />
            </Row>
            <Row>
              <VitalCard label="Respiratory rate" display={vitals.respiratoryRate.display} numericValue={vitals.respiratoryRate.value} thresholdKey="respiratoryRate" icon={RespiratoryRateIcon} />
              <VitalCard label="Blood Glucose" display={vitals.bloodGlucose.display} numericValue={vitals.bloodGlucose.value} thresholdKey="bloodGlucose" icon={BloodGlucoseIcon} />
            </Row>
            <Row>
              <VitalCard label="Temperature" display={vitals.temperature.display} numericValue={vitals.temperature.value} thresholdKey="temperature" icon={TemperatureIcon} />
              <VitalCard label="AVPU Score" display={vitals.avpu.display} numericValue={vitals.avpu.value} thresholdKey="avpu" forceVariant="blue" icon={AVPUIcon} />
            </Row>
            <Row>
              <VitalCard label="Skin Colour" display={vitals.skinColour.display} icon={SkinColorIcon} sx={{ flex: "0 0 calc(50% - 3px)", maxWidth: "calc(50% - 3px)" }} />
            </Row>
          </Box>
